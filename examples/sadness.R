...
library(haven)
nhis_raw <- read_dta("https://github.com/tvogl/econ121/raw/main/data/nhis2010.dta")

summary(nhis_raw)

# drop observations with sadness missing.
# generate a variable that equals one if any bacon consumption, zero otherwise.
nhis <- nhis_raw[!is.na(nhis_raw$asad),]
table(nhis$asad)
nhis$anysad <- ifelse(nhis$asad > 0, 1, 0)
...
library(estimatr)
ols_model <- lm_robust(anysad ~ edyrs + age + male + black + hisp + asian + other + married,data = nhis)
summary(ols_model)
nhis$p_ols <- predict(ols_model, nhis, type="response")

probit_model <- glm(anysad ~ edyrs + age + male + black + hisp + asian + other + married, data = nhis, family=binomial(link="probit"))
summary(probit_model)
nhis$p_probit <- predict(probit_model, nhis, type="response")

logit_model <- glm(anysad ~ edyrs + age + male + black + hisp + asian + other + married, data = nhis, family=binomial(link="logit"))
summary(logit_model)
...
logitmfx(anysad ~ edyrs + age + male + black + hisp + asian + other + married, data = nhis, atmean = TRUE, robust = TRUE)
probitmfx(anysad ~ edyrs + age + male + black + hisp + asian + other + married, data = nhis, atmean = TRUE, robust = TRUE)

#average of the individual marginal effects
logitmfx(anysad ~ edyrs + age + male + black + hisp + asian + other + married, data = nhis, atmean = FALSE, robust = TRUE)
probitmfx(anysad ~ edyrs + age + male + black + hisp + asian + other + married, data = nhis, atmean = FALSE, robust = TRUE)

# ODDS RATIOS

# finally, we can also estimate odds ratios in the logit setting.
# the mfx package also allows estimation of odds ratios:
logitor(anysad ~ edyrs + age + male + black + hisp + asian + other + married, data = nhis, robust = TRUE)
# these are especially convenient for binary independent variables.
# for instance, men have 31% lower odds of reporting sadness than women.